...
| `/shop/diffusers` | Hardcoded slug list (`Kolkata-tea-diffuser`, `coffee-ceramic-diffuser-set`, `black-kitty-terracotta-diffuser`) rendered as image-led editorial feature rows with serif index numbers. | Other products on the bridge (wax melts, fragrance oils) stay reachable elsewhere; only this editorial page narrows. Update the const in `DiffusersPageClient.tsx` to add/remove. Hero image / alt / position read from the bridge-page record. |
