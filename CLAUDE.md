...
- **Object storage is Vercel Blob in prod.** `STORAGE_DRIVER=vercel-blob` is set across all three Vercel scopes for `seijaku-backend`; `BLOB_READ_WRITE_TOKEN` is injected by the connected `seijaku-media-prod` Blob store. Local dev still defaults to `local` (writes to `backend/uploads/`). S3 driver is still wired and selectable via env if we ever migrate off Vercel Blob.
